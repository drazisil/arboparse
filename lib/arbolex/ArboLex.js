...
class ArboLex {
  ...
  _getToken(input, cursor, currentToken) {
    ...
    const rightBracePunctuatorCodes = ["u007d"];

    const singleStringCharactersCodes = ["u0027"];
    ...
    while (
      ...
      !singleStringCharactersCodes.includes(checkUnicode) &&
      !rightBracePunctuatorCodes.includes(checkUnicode)
    ) {
